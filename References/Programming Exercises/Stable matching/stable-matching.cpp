...
bool check_better(int applicant, int new_pos, int old_pos, int n, const int *a_pref)
{
    for(int i = 0; i < n; i ++) {
...
void solver(int n, const int *a_pref, const int *b_pref, int *s)
{
    ...
    while(!positions.empty()) {
        ...
        positions.pop();
        for(int applicant_order = 0; applicant_order < n; applicant_order++) {
            int applicant = b_pref[position*n+applicant_order];
            if(s[applicant] == -1) {
                s[applicant] = position;
                p_matched[position] = true;
                break;
            } else {
                bool better = check_better(applicant, position, s[applicant], n, a_pref);
                if(better) {
                    positions.push(s[applicant]);
                    p_matched[s[applicant]] = false;
                    s[applicant] = position;
                    p_matched[position] = true;
                    break;
                }
