...
// This function return the difference between the number of not included elements and the number of included element of one set.
int elementNotIncluded_difference(bool *elementNotIncluded, pair<int, int> nextSet, const int *p, const int *f) {
    int numberOfUnincludedElements = 0;
    for (int i = p[nextSet.second]; i < p[nextSet.second + 1]; i++) {
        if (elementNotIncluded[f[i]]) {
            numberOfUnincludedElements++;
        }   
    }
    return (nextSet.first - numberOfUnincludedElements);
}

// If m = 2, S0 = {4,6,8} and S1 = {7,9}, then f = [4,6,8,7,9] and p = [0, 3, 5]
// => size(p) = m + 1
void solver(int n, int m, const int *p, const int *f, int &k, int *r)
{
    // The priority queue ranks the pair. 
    // A pair's first element is the number of nonincluded elements for set of index i
    // and the second number is the set index (0 to m - 1)
    // By default a max heap is created ordered by first element of pair.
    // => This priority queue ranks the sets by their number of non-included elements, from max to min
    priority_queue<pair<int, int>> priorityQueue; 

    // elementNotIncluded[i] is true if the element i is not included yet
    bool *elementNotIncluded = new bool[n];    
    
    // when an element is added, this will increment
    int numberOfIncludedElements = 0;

    // Keep track of how many newly included elements in each iteration
    int numberOfAddedElements = -1;                           
    k = 0;

    // Originally, all elements are not included
    for(int i = 0; i < n; i++) 
        elementNotIncluded[i] = true;

    // size(p) = m + 1, so p[i + 1] is actually the starting index of the next set, if it exists. 
    // p[i+1] - p[i] returns the size of the each set, which happens to be number of unincluded elements initially
    for(int i = 0; i < m; i++) {
        priorityQueue.push(make_pair((p[i+1] - p[i]), i));
    }

    while (numberOfIncludedElements != n) {
        // When no set cover exists, set k equal to 0
        if (numberOfAddedElements == 0 || priorityQueue.empty()) {
            k = 0;
            delete[] elementNotIncluded;
            return;
        }
        auto currentSetIndex = priorityQueue.top().second; // The best set with most unincluded elements. This is where the algorithm is greedy
        priorityQueue.pop();
        r[k] = currentSetIndex;
        k++;
        numberOfAddedElements = 0;
        for(int i = p[currentSetIndex]; i < p[currentSetIndex + 1]; i++) { // Loop through all element of the chosen set. For each element, find whether that element is in the elementNotIncluded vector, remove if it does
            if (elementNotIncluded[f[i]]) {
                elementNotIncluded[f[i]] = false;
                numberOfIncludedElements++;
                numberOfAddedElements++;
            }
        }
        auto nextSet = priorityQueue.top();
        int difference = elementNotIncluded_difference(elementNotIncluded, nextSet, p, f); 
        while (difference != 0) {                                                 // Loop while the first element in the queue is not up to date
            priorityQueue.pop();
            priorityQueue.push(make_pair((nextSet.first - difference), nextSet.second));
            nextSet = priorityQueue.top();
            difference = elementNotIncluded_difference(elementNotIncluded, nextSet, p, f);
        }
    }
    delete[] elementNotIncluded;
    return;
