...
/******************************************************************************
  ...
  matching between $n$ applicants and $n$ freePositions. Each applicant
  ...
  the $n$ available freePositions in order
...
bool check_better_position(int applicant, int new_pos, int old_pos, int n, const int *a_pref)
{
    for(int i = 0; i < n; i ++) {
...
void solver(int n, const int *a_pref, const int *b_pref, int *s)
{
    ...
    int *p_matched = new int[n];

    while(!freePositions.empty()) {
        int position = freePositions.front();
        freePositions.pop();
        for(int i = 0; i < n; i++) {
            int applicant = b_pref[position*n+i];
            if(s[applicant] == -1) {
                s[applicant] = position;
                p_matched[position] = applicant;
                break;
            } else {
                bool check_better = check_better_position(applicant, position, s[applicant], n, a_pref);
                if (check_better) {
                    freePositions.push(s[applicant]);
                    p_matched[s[applicant]] = -1;
                    s[applicant] = position;
                    p_matched[position] = applicant;
                    break;
                }
