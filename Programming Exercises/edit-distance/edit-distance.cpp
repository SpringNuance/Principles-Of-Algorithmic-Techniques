    ...
    for(int i = 0; i <= n; i++) {
        for(int j = 0; j <= m; j ++) {
            if (i == 0) {
                ...
                result[i % 2][j] = result[(i - 1) % 2][j-1];
            } else {
                int remove = result[(i - 1) % 2][j];
                int insert = result[i % 2][j - 1];
                int replace = result[(i - 1) % 2][j - 1];
                int mid;
                if (remove + 1 < insert + 1) {
                    mid = remove + 1;
                } else {
                    mid = insert + 1;
                }
